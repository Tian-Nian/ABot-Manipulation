...
python scripts/process_data.py ${task_name} ${env_cfg} ${repo_id} ${mode} ${instruction}
...
bash eval.sh ${task_name} ${env_cfg} ${expert_data_num} ${action_type} ${gpu_id} ${seed} ${policy_conda_env} ${eval_env_conda_env} ${MODEL_PATH} ${TRAIN_CONFIG_NAME} ${REPO_ID}
```